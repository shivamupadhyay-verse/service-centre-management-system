...
def showengineersupdate():
    ...
    def updatedata():
        ...
        sql="update engineers set ename='%s',email='%s',phoneno=%d,catid=%d where engid=%d"%(xb,xc,xd,xe,xa)
        cur.execute(sql)
        db.commit()
        db.close()
    ...
    def fillid():
        ...
        e1['values']=xb
        db.close()
    hd=Label(t,text='engineers update Form',font=('arial',25),fg='black',bg='orange')
    hd.place(x=250,y=5)
    ...
    bt=Button(t,text='update',font=('arial',12,'bold'),fg='white',bg='cyan3',width=10,bd=4,command=updatedata)
    bt.place(x=50,y=350)
    def cancel():
        t.destroy()
